...
/// Converts a memory quantity into LXD's `limits.memory` value.
///
/// - IEC-suffixed quantities (`"512Mi"`, `"4Gi"`) get a `B` appended
///   (`"512Mi"` → `"512MiB"`).
/// - Bare decimal integers (`"536870912"`) are valid Kubernetes quantities
///   (raw bytes) and are passed through as-is; LXD accepts them directly.
pub fn memory_limit_to_lxd(quantity: &str) -> Result<String, LxdError> {
    ...
    // A bare decimal integer is a valid Kubernetes memory quantity (raw bytes).
    ...
    Err(invalid(
        quantity,
        &format!("expected a {suffix_list} suffix (e.g. \"512Mi\", \"4Gi\") or a bare byte count"),
    ))
}
...
mod tests {
    ...
    #[test]
    fn memory_bare_bytes_pass_through() {
        assert_eq!(memory_limit_to_lxd("536870912").unwrap(), "536870912");
        assert_eq!(memory_limit_to_lxd("0").unwrap(), "0");
    }
