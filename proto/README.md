...
`v0.0.116`, the same release the conformance suite (`scripts/conformance.sh`)
runs against, so the driver implements exactly the contract of the gateway
it is tested with.
...
`scripts/conformance.sh` in the same change. Preserve the original `SPDX-FileCopyrightText` /
