...
//! `lxd init --auto` already provides the first two; CI stages the alias
//! before running these tests (see `.github/workflows/static-checks.yaml`);
//! locally: `lxc image alias create lxd-client-test <fingerprint>` against
//! any already-downloaded container image.
