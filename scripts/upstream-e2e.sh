...
# The release's Python SDK is built from the source tree: the protobuf
# modules are generated with GRPCIO_TOOLS_VERSION and the package is
# installed under SDK_SOURCE_VERSION.
...
# The SDK's generated protobuf modules are not committed; generate them with
# grpcio-tools as upstream's own task script does.
...
setup_python() {
    ...
    SETUPTOOLS_SCM_PRETEND_VERSION="$SDK_SOURCE_VERSION" VIRTUAL_ENV="$VENV_DIR" \
        uv pip install --quiet "$SOURCE_DIR" "${PYTHON_TEST_REQUIREMENTS[@]}"
}
